...
def _validate_and_key(upload, slot: str):
    ...
    key = f"portfolio/{slot}/{uuid4().hex}{ext}"
    content_type = getattr(upload, "content_type", None)
    return BytesIO(data), key, content_type
...
async def save_uploaded_image(file: UploadFile, prefix: str = "portfolio") -> str:
    """
    Upload an image to private Backblaze B2.

    Returns the B2 object key, NOT a public URL.
    """
...
async def update_project(
    ...
    if photo and photo.filename:
        ...
        project.image_path = await save_uploaded_image(
            photo,
            prefix="portfolio/projects"
        )
    
        if old_image and not old_image.startswith("/static/"):
            try:
                delete_object(old_image)
...
def images_page(
    ...
    profile_images = db.query(models.SiteImage).filter_by(
        slot="profile"
    ).order_by(models.SiteImage.uploaded_at.desc()).all()

    work_images = db.query(models.SiteImage).filter_by(
        slot="work"
    ).order_by(models.SiteImage.uploaded_at.desc()).all()

    for image in profile_images:
        try:
            image.image_url = signed_url(image.file_path)
        except B2StorageError:
    ...
    if image:
        ...
        if image_path and not image_path.startswith("/static/"):
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Failed to delete image from B2: {exc}"
                ) from exc
        
    else:
        # Legacy local image
        file_on_disk = image_path.lstrip("/")
